...
export default function CoursesPage() {
    ...
    async function fetchCourses() {
      try {
        ...
        const mapped = res.data.map((c: any) => ({
          ...c,
          title: c.course_name,
          image: c.image || 'https://images.unsplash.com/photo-1516321318423-f06f85e504b3?w=800&h=450&fit=crop',
          tags: ['Draft'],
          status: 'published',
          visibility: 'everyone',
        }));
        ...
        console.error("Failed to load courses from backend, reverting to local mock database", err);
        const mappedMocks = mockCourses.map(c => ({
          ...c,
          title: c.course_name,
        }));
        setCourses(mappedMocks);
      }
    }
    fetchCourses();
  }, []);
  ...
  const filteredCourses = courses.filter((course) => {
    ...
    // Tag filter
    if (selectedTag !== 'All' && !(course.tags && course.tags.some((tag: string) => tag === selectedTag || (selectedTag === 'Mobile' && tag === 'Flutter')))) return false;
