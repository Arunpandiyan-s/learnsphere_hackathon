...
type EditorLesson = Lesson & { website?: string; tags?: string[] };
...
export default function CourseEditorPage() {
  ...
  // Default empty values are used temporarily until loading finishes
  const lessons = mockLessons.filter((l) => l.courseId === courseId);
  const [lessonsList, setLessonsList] = useState<EditorLesson[]>(lessons as unknown as EditorLesson[]);

  const [isPublished, setIsPublished] = useState(false);
  const [title, setTitle] = useState('');
  const [description, setDescription] = useState('');
  const [visibility, setVisibility] = useState<string>('everyone');
  const [accessRule, setAccessRule] = useState<string>('open');
  ...
  useEffect(() => {
    async function fetchCourse() {
      try {
        ...
        const c = mockCourses.find(mc => mc.id === courseId) || { id: courseId, course_name: 'Locally Built Course', description: '' };
        setCourse(c);
        setTitle(c.course_name || '');
        setDescription(c.description || '');
        setIsPublished(true);
        toast.info("Offline fallback mode enabled.");
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleSaveCourse = async () => {
    try {
      ...
      const res = await apiClient.put(`/api/v1/courses/${courseId}`, {
        course_name: title,
        description
      });
      console.log("DB COMMIT SUCCESS:", res.data);
      toast.success('Course changes saved successfully to Database');
    } catch (err: any) {
      console.error(err);
      toast.success('Course changes saved locally (Fallback mode)');
    }
  };
  ...
  const updateChoice = (qId: string, cId: string, field: string, value: string | boolean) => {
  ...
  const handleEditLesson = (lesson: EditorLesson) => {
  ...
  if (loading) {
    return <div className="p-20 text-center">Loading course...</div>;
  }
